...
hubgenes <-intersect(intersect(C,D),CRRS)
length(intersect(C,CRRS))
length(intersect(B$Tag,CRRS))
length(intersect(C,CRRS))
length(intersect(intersect(A$Tag,B$Tag),C))
length(intersect(intersect(B$Tag,C),CRRS))
length(intersect(intersect(A$Tag,C),CRRS))
length(intersect(intersect(B$Tag,C),CRRS))
...
venn.plot <- draw.triple.venn(
  ...
  n23 = 2,
  n123 = 1,
  category = c("TCGA DEGs","FUSCC DEGs",  "CRRS"),
...
PP <- ggboxplot(hubdat,x = 'group',y = "P2RX1",fill='group')+theme_classic()+ylab('P2RX1 Expression')+xlab('')+ggtitle('TCGA')+ylim(min(hubdat$P2RX1)-0.2,max(hubdat$P2RX1)+0.2)+
  scale_fill_manual(values=c("#FFD700", "#BF3EFF"))+
...
ggsave('figures/07D_P2RX1_TCGA.png',PP,width = 5,height =5,dpi=200)

#######################CNV
cnv_tcga <- read.table("rawdata/TCGA_LUAD/TCGA_CNA.txt",header = T,row.names = 1,sep='\t')
cnv_tcga <- cnv_tcga[rownames(cnv_tcga)=='P2RX1',]
cnv_tcga <- cnv_tcga %>% t %>% as.data.frame()
cnv_tcga$sample <- rownames(cnv_tcga)
cnv_tcga$group <- ifelse(cnv_tcga$P2RX1>0,'Gain',ifelse(cnv_tcga$P2RX1==0,'Diploid','Loss'))
cnv_tcga$sample <- gsub('\\.','-',cnv_tcga$sample)
cnv_tcga <- cnv_tcga[cnv_tcga$sample%in%lxdata1$sampleID,]
...
pp <- ggboxplot(pd,x = 'Gene', y='Composition',fill='group')+theme_bw()+ylab('Methylation')+xlab('TCGA')+ylim(min(pd$Composition)-1,max(pd$Composition)+2)+
  ...
  theme(axis.text.x = element_text(size=18,face='bold',color='black'),
        ...
        axis.line = element_line(size=1),
        panel.grid = element_blank(),panel.border = element_rect(size=2))
ggsave('figures/06C_meth_TCGA.png',pp,width = 5,height = 5,dpi=200)

PP <- ggboxplot(hub_meth1,x = 'group',y = 'P2RX1',fill='group')+theme_classic()+ylab('Methylation')+xlab('')+ggtitle('TCGA')+ylim(min(hub_meth1$P2RX1)-1,max(hub_meth1$P2RX1)+1)+
  scale_fill_manual(values=c("orange", "darkgreen"))+
  geom_signif(size=1,textsize=12,comparisons = list(c('Tumor','Normal')),test = wilcox.test,map_signif_level = T)+
  theme(plot.title = element_text(hjust = 0.5,vjust = 1.5,face = 'bold',size = 25),
        axis.text.x = element_text(size=18,face='bold',color='black'),
        ...
        legend.title = element_blank(),legend.position = "none",
        axis.line = element_line(size=1))
ggsave('figures/07C_P2RX1meth_TCGA.png',PP,width = 5,height = 5,dpi=200)
...
png("figures/07G_OSKM_P2RX1_TCGA.png",width = 1500,height = 1500,res=200)
...
TMB$group <- ifelse(TMB$Expression>median(TMB$Expression),'High P2RX1','Low P2RX1')
TMB <- TMB[TMB$sample%in%lxdata1$sampleID,]
write.xlsx(TMB,file = 'files/Fig6.xlsx',sheetName = 'Fig6F_TMB',row.names = F,append = T)

pp <- ggboxplot(TMB,x = 'group',y = 'TMB',fill='group')+theme_classic()+ylab('TMB')+xlab('')+ggtitle('TCGA')+ylim(min(TMB$TMB)-3,max(TMB$TMB)+5)+
  scale_fill_manual(values=c("#FFD700", "#BF3EFF"))+
  geom_signif(size=1,textsize=12,comparisons = list(c('High P2RX1','Low P2RX1')),test = wilcox.test,map_signif_level = T)+
  theme(plot.title = element_text(hjust = 0.5,vjust = 1.5,face = 'bold',size = 25),
        axis.text.x = element_text(size=18,face='bold',color='black'),
        axis.text.y = element_text(size=18,face='bold',color='black'),
        text=element_text(size=18,face='bold'),
        legend.title = element_blank(),legend.position = "none",
        axis.line = element_line(size=1))
ggsave('Cfigures/07F_TMB_TCGA.png',pp,width = 5,height = 5,dpi=200)

ESTIMATE$group <- ifelse(ESTIMATE$exp>median(ESTIMATE$exp),'High P2RX1','Low P2RX1')
...
imm <- ggboxplot(ESTIMATE,x = 'group',y = 'ImmuneScore',fill='group')+theme_classic()+ylab('ImmuneScore')+xlab('')+ggtitle('TCGA')+ylim(min(ESTIMATE$ImmuneScore)-1000,max(ESTIMATE$ImmuneScore)+1000)+
  scale_fill_manual(values=c("#FFD700", "#BF3EFF"))+
  geom_signif(size=1,textsize=12,comparisons = list(c('High P2RX1','Low P2RX1')),test = wilcox.test,map_signif_level = T)+
  theme(plot.title = element_text(hjust = 0.5,vjust = 1.5,face = 'bold',size = 25),
        axis.text.x = element_text(size=18,face='bold',color='black'),
        ...
        legend.title = element_blank(),legend.position = "none",
        axis.line = element_line(size=1))
ggsave('figures/07F_imm_TCGA.png',imm,width = 5,height = 5,dpi=200)

strom <- ggboxplot(ESTIMATE,x = 'group',y = 'StromalScore',fill='group')+theme_classic()+ylab('StromalScore')+xlab('')+ggtitle('TCGA')+ylim(min(ESTIMATE$StromalScore)-1000,max(ESTIMATE$StromalScore)+1000)+
  scale_fill_manual(values=c("#FFD700", "#BF3EFF"))+
  geom_signif(size=1,textsize=12,comparisons = list(c('High P2RX1','Low P2RX1')),test = wilcox.test,map_signif_level = T)+
  theme(plot.title = element_text(hjust = 0.5,vjust = 1.5,face = 'bold',size = 25),
        axis.text.x = element_text(size=18,face='bold',color='black'),
        ...
        legend.title = element_blank(),legend.position = "none",
        axis.line = element_line(size=1))
ggsave('figures/07F_strom_TCGA.png',strom,width = 5,height = 5,dpi=200)

est <- ggboxplot(ESTIMATE,x = 'group',y = 'ESTIMATEScore',fill='group')+theme_classic()+ylab('ESTIMATEScore')+xlab('')+ggtitle('TCGA')+ylim(min(ESTIMATE$ESTIMATEScore)-1000,max(ESTIMATE$ESTIMATEScore)+1000)+
  scale_fill_manual(values=c("#FFD700", "#BF3EFF"))+
  geom_signif(size=1,textsize=12,comparisons = list(c('High P2RX1','Low P2RX1')),test = wilcox.test,map_signif_level = T)+
  theme(plot.title = element_text(hjust = 0.5,vjust = 1.5,face = 'bold',size = 25),
        axis.text.x = element_text(size=18,face='bold',color='black'),
        ...
        legend.title = element_blank(),legend.position = "none",
        axis.line = element_line(size=1))
ggsave('figures/07F_est_TCGA.png',est,width = 5,height = 5,dpi=200)
library(xlsx)
scores <- read.xlsx('figures/05_data.xlsx',row.names=T,sheetIndex = 'norm_ssGSEA_Score')
scores$sampleID <- rownames(scores)
scores <- merge(scores,lxdata1[,c('sampleID',"P2RX1")],by.x='sampleID')
s <- c()
for(i in 2:29){
  cs <- cor(scores$P2RX1,scores[i])
  s[i-1] <- cs
}

ss <- data.frame(immuecell=colnames(scores)[2:29],cor=s)
ss <- ss[order(ss$cor,decreasing = T),]
barplot(ss$cor,names.arg = ss$immuecell)

library(corrplot)
library(ggstatsplot)

p1 <- ggscatterstats(scores, 
                     x = "P2RX1", 
                     y = "Act_B");p1
ggsave('figures/07_Act_B.png',p1,width = 5,height = 5,dpi=200)

p2 <- ggscatterstats(scores, 
                     x = "P2RX1", 
                     y = "Th2");p2
ggsave('figures/07_Th2.png',p2,width = 5,height = 5,dpi=200)
