...
clin$Status <- ifelse(clin$status=='A',0,1)
clin$status <-  ifelse(clin$status=='A','Alive','Dead')
clin$Time <-as.numeric(clin$time)
# colnames(clin) <- gsub('-','_',colnames(clin) )
# clin$P2RX1[is.na(clin$P2RX1)] <- 0
clin$Risk=-0.299*clin$P2RX+0.107*clin$PLAUR-0.124*clin$PRKCZ+0.093*clin$F12+0.056*clin$SERPINE1+
...
library(survival)
library(survminer)
png("CRG/result/figs/05_OSKM_risk_GSE42127.tiff",width = 1500,height = 1500,res=200)
...
library(ggpubr)
#风险评分box图
cb <- ggboxplot(clin,x = 'status',y = 'Risk',fill='status')+theme_bw()+ylab('Riskscore')+xlab('')+ggtitle('GSE42127')+ylim(-1,3)+
...
ggsave('CRG/result/figs/05_riskbox.tiff',cb,width = 5,height = 5,dpi=200)

#点图
pd <- clin[order(clin$Risk,decreasing = F),]
pd$Sample <- 1:nrow(pd)
sp <- ggplot(pd,aes(Sample,Time,color=status))+geom_point()+geom_vline(aes(xintercept = 67),lty=2,lwd=1)+
  ...
  theme(axis.text.x = element_text(size=16,face='bold',color='black'),
        ...
        legend.title = element_blank(),legend.position = "none",
        panel.grid = element_blank(),panel.border = element_rect(size=1.5))
rp <- ggplot(pd,aes(Sample,Risk,color=Group))+geom_point()+geom_vline(aes(xintercept = 67),lty=2,lwd=1)+
  theme_bw()+scale_colour_manual(values = c('red','blue'))+ylab('Riskscore')+xlab('')+ggtitle('GSE42127')+
  theme(plot.title = element_text(hjust = 0.5,vjust = 1.5,face = 'bold',size = 25),axis.text.x = element_text(size=18,face='bold',color='black'),
        ...
        panel.grid = element_blank(),panel.border = element_rect(size=1.5))
ggsave('CRG/result/figs/05_survpoint.tiff',sp,width = 5,height =3,dpi=1000)
ggsave('CRG/result/figs/05_riskpoint.tiff',rp,width = 5,height = 3,dpi=1000)
...
save_pheatmap_pdf(p, "CRG/result/figs/05_pheat.pdf")
save(pd,file = 'CRG/extdata/GSE42127_surv133.Rdata')
load( 'CRG/extdata/GSE42127_surv133.Rdata')
...
png("CRG/result/figs/05_rocyear_risk_GSE42127.tiff",width = 1500,height = 1500,res=200)
plot(troc.1$FP, troc.1$TP, type="l",col="red", xlim=c(0,1), ylim=c(0,1), lwd=4,cex.axis=1.5,ann = F)
title(main='GSE42127',xlab="1-Specificity", ylab="Sensitivity",cex.main = 2.5, font.main= 2,cex.lab=1.6,font.lab=2)
...
png("CRG/result/figs/03_rocmethods_bsr_GSE42127.tiff",width = 1500,height = 1500,res=200)
plot(troc.1$FP, troc.1$TP, type="l",col="red", xlim=c(0,1), ylim=c(0,1), lwd=4,cex.axis=1.5,ann = F)
title(main='GSE42127',xlab="1-Specificity", ylab="Sensitivity",cex.main = 2.5, font.main= 2,cex.lab=1.6,font.lab=2)
abline(0,1,col='gray',lty=2,lwd=3)
...
legend(0.55,0.4,c(paste('Risk=',round(troc.1$AUC,3)),
       ...
       x.intersp =1,y.intersp =1,text.font = 4,lty = 1,lwd=4,col = c('red',"green",'orange','black',"yellow","pink","cyan","magenta","red4"),
       bty='n',seg.len = 1.5,cex = 1.2)
