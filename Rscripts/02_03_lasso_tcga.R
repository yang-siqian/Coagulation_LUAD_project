...
# Fig2B
png("figures/02B_lassocoef.tiff",width = 1500,height = 1500,res=200)
...
# Fig2A
png("figures/02A_coefficient.tiff",width = 1500,height = 1500,res=200)
...
library(My.stepwise)
bsrdata <- CRGs_surv_dat[,c('OS','OS.E',lasso_genes)]
...
library(xlsx)
write.xlsx(dmf,file='files/Fig2.xlsx',sheetName = 'Fig2C',row.names = F)
# Fig2C
library(ggalt)
...
ggsave('figures/02C_dot_coef_bsr.tiff',dp,width = 6,height = 6,dpi=1000)
...
png("figures/02F_rocmethods_bsr_TCGA.tiff",width = 1500,height = 1500,res=200)
...
write.xlsx(dat1,file = 'files/Fig2.xlsx',sheetName = 'Fig2E',row.names = F,append = T)
...
cox9 <- coxph(Surv(OS, OS.E) ~ COL1A2, data = dat1,x=T,y=T)
...
png("figures/02E_cindex.tiff",width = 1500,height = 1500,res=200)
...
# Fig3C
png("figures/03C_rocyear_risk_TCGA.tiff",width = 1500,height = 1500,res=200)
plot(troc.1$FP, troc.1$TP, type="l",col="red", xlim=c(0,1), ylim=c(0,1), lwd=4,cex.axis=1.5,ann = F)
title(main='TCGA',xlab="1-Specificity", ylab="Sensitivity",cex.main = 2.5, font.main= 2,cex.lab=1.6,font.lab=2)
abline(0,1,col='gray',lty=2,lwd=3)
...
png("figures/03D_OSKM_risk_TCGA.tiff",width = 1500,height = 1500,res=200)
...
png("figures/03E_RFSKM_risk_TCGA.tiff",width = 1500,height = 1500,res=200)
...
ggsave('figures/03B_riskbox.tiff',cb,width = 5,height = 5,dpi=200)
...
rp <- ggplot(dat4,aes(Sample,Risk,color=group))+geom_point()+geom_vline(aes(xintercept = 251),lty=2,lwd=1)+
  ...
  theme(plot.title = element_text(hjust = 0.5,vjust = 1.5,face = 'bold',size = 25),axis.text.x = element_text(size=18,face='bold',color='black'),
        ...
        panel.grid = element_blank(),panel.border = element_rect(size=1.5))
ggsave('figures/03A_survpoint.tiff',sp,width = 5,height =3,dpi=1000)
ggsave('figures/03A_riskpoint.tiff',rp,width = 5,height = 3,dpi=1000)
