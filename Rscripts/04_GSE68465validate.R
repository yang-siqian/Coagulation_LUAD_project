...
library(survival)
library(survminer)
png("CRG/result/figs/05_OSKM_risk_GSE68465.tiff",width = 1500,height = 1500,res=200)
...
library(ggpubr)
#风险评分box图
cb <- ggboxplot(clin,x = 'status',y = 'Risk',fill='status')+theme_bw()+ylab('Riskscore')+xlab('')+ggtitle('GSE68465')+ylim(-1,3)+
...
pd$Sample <- 1:nrow(pd)
sp <- ggplot(pd,aes(Sample,Time,color=status))+geom_point()+geom_vline(aes(xintercept = 222),lty=2,lwd=1)+
  ...
  theme(axis.text.x = element_text(size=16,face='bold',color='black'),
        ...
        legend.title = element_blank(),legend.position = "none",
        panel.grid = element_blank(),panel.border = element_rect(size=1.5))
rp <- ggplot(pd,aes(Sample,Risk,color=Group))+geom_point()+geom_vline(aes(xintercept = 222),lty=2,lwd=1)+
  theme_bw()+scale_colour_manual(values = c('red','blue'))+ylab('Riskscore')+xlab('')+ggtitle('GSE68465')+
...
load( 'CRG/extdata/GSE68465_surv443.Rdata')
...
png("CRG/result/figs/05_rocyear_risk_GSE68465.tiff",width = 1500,height = 1500,res=200)
plot(troc.1$FP, troc.1$TP, type="l",col="red", xlim=c(0,1), ylim=c(0,1), lwd=4,cex.axis=1.5,ann = F)
title(main='GSE68465',xlab="1-Specificity", ylab="Sensitivity",cex.main = 2.5, font.main= 2,cex.lab=1.6,font.lab=2)
...
png("CRG/result/figs/03_rocmethods_bsr_GSE68465.tiff",width = 1500,height = 1500,res=200)
plot(troc.1$FP, troc.1$TP, type="l",col="red", xlim=c(0,1), ylim=c(0,1), lwd=4,cex.axis=1.5,ann = F)
title(main='GSE68465',xlab="1-Specificity", ylab="Sensitivity",cex.main = 2.5, font.main= 2,cex.lab=1.6,font.lab=2)
abline(0,1,col='gray',lty=2,lwd=3)
