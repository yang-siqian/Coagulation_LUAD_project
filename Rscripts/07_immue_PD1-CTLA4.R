...
response <- read_xlsx('LUADdata/TCGA/Gide2019_PD1+CTLA4_Melanoma_RNASeq/mmc3.xlsx')
...
Gide <- read.table('LUADdata/TCGA/Gide2019_PD1+CTLA4_Melanoma_RNASeq/ICB.Gide2019_Pembrolizumab-Nivolumab+Ipilimumab_Melanoma.self_subtract',header = T,row.names = 1)
...
png("CRG/result/figs/09_OSKM_risk_PRJEB23709_PD1_CTLA4.tiff",width = 1500,height = 1500,res=200)
...
png("CRG/result/figs/09_PFSKM_risk_PRJEB23709_PD1_CTLA4.tiff",width = 1500,height = 1500,res=200)
...
png("CRG/result/figs/09_OSKM_risk_response_PD1_CTLA4.tiff",width = 1500,height = 1500,res=200)
p <- ggsurvplot(survfit(Surv(OS,OS.E) ~ Group, data=survdata),size=3,
                surv.median.line = "hv",
                title = "PRJEB23709_PD1_CTLA4",
...
png("CRG/result/figs/09_PFSKM_risk_response_PD1_CTLA4.tiff",width = 1500,height = 1500,res=200)
p <- ggsurvplot(survfit(Surv(PFS, PFS.E) ~ Group, data=survdata),size=3,
                surv.median.line = "hv",
                title = "PRJEB23709_PD1_CTLA4",
                legend.labs = c('CR/PR','SD/PD'),legend=c(0.8,0.96),font.legend=c(20,'bold'),
...
cb <- ggboxplot(survdata,x = 'Group',y = 'Risk',fill='Group')+theme_bw()+ylab('Riskscore')+xlab('')+ggtitle('PRJEB23709_PD1_CTLA4')+ylim(-1.5,1.3)+
...
ggsave('CRG/result/figs/09_riskbox_PRJEB23709_PD1_CTLA4.tiff',cb,width = 5,height = 5,dpi=200)
library(reshape2)
table(survdata$Group[survdata$group=='High risk'])
table(survdata$Group[survdata$group=='Low risk'])
a <-data.frame(group=c('High risk','Low risk'),CR_PR=c(56.3,75),SD_PD=c(43.7,25))
b=melt(a,id.vars='group')
b$variable <- gsub('_','/',b$variable)
P <- ggplot(data=b,aes(x=group,y=value,fill=variable)) + theme_bw()+ylab('Percent')+xlab('')+ggtitle('PRJEB23709_PD1_CTLA4')+ylim(0,100)+
  scale_fill_manual(values=c("#E69F00", "#56B4E9"))+
  ...
  theme(plot.title = element_text(hjust = 0.5,vjust = 1.5,face = 'bold',size = 25),
        ...
        panel.grid = element_blank(),panel.border = element_rect(size=1.5))
ggsave('CRG/result/figs/09_barplot_PRJEB23709_PD1_CTLA4.tiff',P,width = 5.2,height = 5.2,dpi=200)
...
png("CRG/result/figs/09_rocyear_risk_PD1_CTLA4.tiff",width = 1500,height = 1500,res=200)
...
png("CRG/result/figs/09_roc_risk_PD1-CTLA4.tiff",width = 1500,height = 1500,res=200)
