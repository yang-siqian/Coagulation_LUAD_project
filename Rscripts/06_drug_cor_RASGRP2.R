...
write.table(outTab, file= "CRG/result/files/RASGRP2drugCor.txt", sep= "\t", row.names=F, quote=F)
...
dd_c2$group <- ifelse(dd_c2$RASGRP2>median(dd_c2$RASGRP2),'High','Low')
...
ggsave('CRG/result/figs/02_drug_sensitivity.tiff',PP,width = 16,height = 10,dpi=200)
...
outTab1$label <- paste0('cor=',outTab1$cor,' ',outTab1$pval)
outTab1$x=rep(3,nrow(outTab1))
outTab1$y=rep(-2,nrow(outTab1))
colnames(outTab1)[2] <- 'Celltype'

cp <- ggplot( data= pd, aes(x = RASGRP2, y = Composition))+ geom_point(size= 1)+ylab("IC50")+xlab( "Expression of RASGRP2")+theme_bw() +
...
ggsave('CRG/result/figs/02_drug_cor.tiff',cp,width = 16,height = 10,dpi=200)
