...
# SupFig2C_GSE13213
library(xlsx)
write.xlsx(clin,file = 'files/SupFig2.xlsx',sheetName = 'SupFig2_GSE13213',row.names = F)
library(survival)
library(survminer)
png("figures/S02C_OSKM_risk_GSE13213.tiff",width = 1500,height = 1500,res=200)
...
rp <- ggplot(pd,aes(Sample,Risk,color=Group))+geom_point()+geom_vline(aes(xintercept = 59),lty=2,lwd=1)+
  ...
  theme(plot.title = element_text(hjust = 0.5,vjust = 1.5,face = 'bold',size = 25),axis.text.x = element_text(size=18,face='bold',color='black'),
        ...
        panel.grid = element_blank(),panel.border = element_rect(size=1.5))
ggsave('figures/S02A_survpoint.tiff',sp,width = 5,height =3,dpi=1000)
ggsave('figures/S02A_riskpoint.tiff',rp,width = 5,height = 3,dpi=1000)
...
save_pheatmap_pdf(p, "figures/S02A_pheat.pdf")
...
png("figures/S02B_rocyear_risk_GSE13213.tiff",width = 1500,height = 1500,res=200)
...
write.xlsx(pd[,c(1,18:20,23:32)],file = 'files/Fig2.xlsx',sheetName = 'Fig2G_GSE13213',row.names = F,append = T)
...
png("figures/02G_rocmethods_bsr_GSE13213.tiff",width = 1500,height = 1500,res=200)
plot(troc.1$FP, troc.1$TP, type="l",col="red", xlim=c(0,1), ylim=c(0,1), lwd=4,cex.axis=1.5,ann = F)
title(main='GSE13213',xlab="1-Specificity", ylab="Sensitivity",cex.main = 2.5, font.main= 2,cex.lab=1.6,font.lab=2)
abline(0,1,col='gray',lty=2,lwd=3)
