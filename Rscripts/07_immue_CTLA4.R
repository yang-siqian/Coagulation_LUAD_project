...
response <- read.table('LUADdata/TCGA/Nathanson2017_CTLA4_Melanoma_RNASeq_Post/ICB.Nathanson2017_Ipilimumab_Melanoma_Post.clinical',header = T)
response$Group <- ifelse(response$Response=='0','SD/PD','CR/PR')
colnames(response)[1] <- 'sample'
Nathanson <- read.table('LUADdata/TCGA/Nathanson2017_CTLA4_Melanoma_RNASeq_Post/ICB.Nathanson2017_Ipilimumab_Melanoma_Post.self_subtract',header = T,sep = '\t')
...
survdata$Time <- survdata$OS*12

png("CRG/result/figs/09_OSKM_risk_Nathanson_CTLA4.tiff",width = 1500,height = 1500,res=200)
p <- ggsurvplot(survfit(Surv(Time,OS.Event) ~ group, data=survdata),size=3,
                surv.median.line = "hv",
...
png("CRG/result/figs/09_OSKM_risk_response_CTLA4.tiff",width = 1500,height = 1500,res=200)
p <- ggsurvplot(survfit(Surv(Time,OS.Event) ~ Group, data=survdata),size=3,
                surv.median.line = "hv",
                title = "Nathanson_CTLA4",
...
cb <- ggboxplot(survdata,x = 'Group',y = 'Risk',fill='Group')+theme_bw()+ylab('Riskscore')+xlab('')+ggtitle('Nathanson_CTLA4')+ylim(-1,1.3)+
...
ggsave('CRG/result/figs/09_riskbox_Nathanson_CTLA4.tiff',cb,width = 5,height = 5,dpi=200)
library(reshape2)
table(survdata$Group[survdata$group=='High risk'])
table(survdata$Group[survdata$group=='Low risk'])
a <-data.frame(group=c('High risk','Low risk'),CR_PR=c(14.3,37.5),SD_PD=c(85.7,62.5))
b=melt(a,id.vars='group')
b$variable <- gsub('_','/',b$variable)
P <- ggplot(data=b,aes(x=group,y=value,fill=variable)) + theme_bw()+ylab('Percent')+xlab('')+ggtitle('Nathanson_CTLA4')+ylim(0,100)+
  scale_fill_manual(values=c("#E69F00", "#56B4E9"))+
  ...
  theme(plot.title = element_text(hjust = 0.5,vjust = 1.5,face = 'bold',size = 25),
        ...
        panel.grid = element_blank(),panel.border = element_rect(size=1.5))
ggsave('CRG/result/figs/09_barplot_Nathanson_CTLA4.tiff',P,width = 5,height = 5,dpi=200)
...
png("CRG/result/figs/09_rocyear_risk_CTLA4.tiff",width = 1500,height = 1500,res=200)
...
png("CRG/result/figs/09_roc_risk_CTLA4.tiff",width = 1500,height = 1500,res=200)
