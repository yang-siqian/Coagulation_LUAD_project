...
###########riskscore  multicox
# Fig7B
mulcox<- coxph(Surv(OS, OS.E) ~ Age+Tumor+Lymph_Node+Metastasis+Risk, data = lxdata1 );mulcox
...
ggsave(filename="figures/07B_muti_forest.png",forest, width=20, height=14)
...
png("figures/07C_nomo.tiff",width = 2500,height = 2000,res=200)
...
# Fig7D
f2 <- psm(Surv(OS, OS.E) ~ Age+Stage+Risk, data = dat,x=T,y=T,dist='lognormal' )
call <- calibrate(f2,cmethod = 'KM',method='boot',u=365,m=164,B=1000)
png("figures/07E_cal_1y.tiff",width = 1500,height = 1500,res=200)
...
png("figures/07E_cal_3y.tiff",width = 1500,height = 1500,res=200)
...
png("figures/07E_cal_5y.tiff",width = 1500,height = 1500,res=200)
...
png("figures/07D_cindex.tiff",width = 1500,height = 1500,res=200)
...
####1-year
coxph(Surv(OS, OS.E) ~ age, data = lxdata1)
coxph(Surv(OS, OS.E) ~ stage, data = lxdata1)
coxph(Surv(OS, OS.E) ~ age+stage+Risk, data = lxdata1 )
lxdata1$age <- ifelse(lxdata$Age=='young',1,2)
lxdata1$stage<- ifelse(lxdata$Stage=='I',1, ifelse(lxdata$Stage=='II',2, ifelse(lxdata$Stage=='III',3, ifelse(lxdata$Stage=='IV',4,0))))
...
png("figures/07F_1-yearroc_combined.tiff",width = 1500,height = 1500,res=200)
png("figures/07F_3-yearroc_combined.tiff",width = 1500,height = 1500,res=200)
png("figures/07F_5-yearroc_combined.tiff",width = 1500,height = 1500,res=200)
...
dev.off()
write.xlsx(lxdata1[,c(1,13:18,31:36)],file = 'files/Fig7.xlsx',sheetName = 'Fig7_clin',row.names = F,append = T)
